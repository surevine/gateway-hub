function languageGraph(element, languages) {
	...
	var arc = d3.svg.arc().outerRadius(radius - 10).innerRadius(radius - 120);

	var pie = d3.layout.pie().sort(null).value(function(d) {
		return d.lines;
	});

	var svg = d3.select(element[0]).append('div').classed('languagesPie', true)
			.classed('metastats', true).append('svg').attr("width", width)
			.attr("height", height).attr('viewBox',
					'0, 0, ' + width + ', ' + height).attr(
					'preserveAspectRatio', 'xMidYMid meet').append("g").attr(
					"transform",
